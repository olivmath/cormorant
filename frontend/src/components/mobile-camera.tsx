...
export function MobileCamera() {
  ...
  async function start() { try { setMessage("Conectando…"); const credentials = await requestLiveKitToken("publisher"); const nextRoom = new Room(); room.current = nextRoom; await nextRoom.connect(credentials.server_url, credentials.token); await nextRoom.localParticipant.setCameraEnabled(true, { facingMode: "environment" }); setLive(true); setMessage("Ao vivo. Mantenha esta página aberta."); } catch { setMessage("Não foi possível iniciar. Verifique a permissão da câmera."); } }
